...
class Drone:
    def __init__(self, x, y):
        ...
        self.__visited = [[False for _ in range(20)] for _ in range(20)]
        self.__visited[x][y] = True

        self.__dirs = [(1, 0), (0, 1), (-1, 0), (0, -1)]
    ...
    def _valid(self, y, x, mapped_surface):
        return 0 <= x < 20 and 0 <= y < 20 and not self.__visited[y][x] and mapped_surface[y, x] == 0

    def moveDSF(self, mapped_surface):
        for direction in self.__dirs:
            new_x, new_y = self.x + direction[0], self.y + direction[1]
            if self._valid(new_y, new_x, mapped_surface):
                self.__positions.append((self.y, self.x))
                self.__positions.append((new_y, new_x))

        if len(self.__positions) == 0:
            self.x, self.y = None, None
            raise SystemExit

        self.y, self.x = self.__positions.pop()
        self.__visited[self.y][self.x] = True
